...
print("Sender Listening for Connection...")
serverSocket.listen()
...
while basestate:
        ...
        for i in range(0, len(data_to_send)):
            if i < client.rate:
                client.data.append(data_to_send[i])
                        # elif i ==(len(data_to_send)+len(buffer.buffer)):
    ...
    clientConnection.send("".join(client.data[-client.rate:]).encode())
    mesg = clientConnection.recv(1024)